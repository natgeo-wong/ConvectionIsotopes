...
function wrf3Dsmooth(
	...
	for ii in (1+buffer) : (ndt-buffer)
		...
		ids = NCDataset(datadir("wrf","3D","$(wvar)-daily.nc"))
	...
	fnc = datadir("wrf","3D","$wvar-daily-smooth_$(@sprintf("%02d",days))days.nc")
...
function wrf2Dsmooth(
    ...
    ds   = NCDataset(datadir("wrf","2D","$(wvar)-daily.nc"))
	lon  = ds["longitude"][:]; nlon = ds.dim["longitude"]
    lat  = ds["latitude"][:];  nlat = ds.dim["latitude"]
    ndt  = ds.dim["date"]; start = ds["time"][1]
	attrib = Dict(ds[wvar].attrib)
	...
	for ii in (1+buffer) : (ndt-buffer)
		...
		ids = NCDataset(datadir("wrf","2D","$(wvar)-daily.nc"))
		NCDatasets.load!(ids[wvar].var,oarr,:,:,ii.+(-buffer:buffer))
		close(ids)
	...
	fnc = datadir("wrf","2D","$wvar-daily-smooth_$(@sprintf("%02d",days))days.nc")
	if isfile(fnc); rm(fnc,force=true) end
