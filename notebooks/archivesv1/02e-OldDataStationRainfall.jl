...
begin
	df = DataFrame(XLSX.readtable(datadir("OldData.xlsx"),"Sample_and_Result_Info")...)
	df = df[:,[
...
begin
	if isfile(datadir("processed-old.nc"))
		rm(datadir("processed-old.nc"),force=true)
	end
	
	nds = NCDataset(datadir("processed-old.nc"),"c")
	nds.dim["station"] = 4
