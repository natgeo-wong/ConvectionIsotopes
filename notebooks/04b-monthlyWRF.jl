...
begin
	const v3D_1 = zeros(Float32,543,543,50,8)
	const v3D_2 = zeros(Float32,543,543,51,8)
	md"Temporary Arrays ..."
end

# ╔═╡ d1c38d2c-49c8-4fe8-a893-9b5704b94a22
begin
	const v3D_p = zeros(Float32,543,543,50)
	const v3D_z = zeros(Float32,543,543,51)
	const v3D_w = zeros(Float32,543,543,51)
	for ifol = 1 : 3
		dsvec = glob(
			"wrfout3D_*",
			datadir("wrf","wrfout","$(uppercase(monthabbr(imo)))p0$ifol")
