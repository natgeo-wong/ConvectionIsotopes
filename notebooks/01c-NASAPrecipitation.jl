...
for it = 1 : 48
	...
	c2 = a2[1].pcolormesh(
		...
		levels=10. .^(-1:0.1:0),cmap="blues",extend="both"
	)
	a2[1].plot(x,y,lw=1,c="k")
	a2[1].plot(slon_PCS,slat_PCS)
	a2[1].plot(slon_PCS,slat_PCS)
	a2[1].plot(slon_SAN,slat_SAN)
	a2[1].format(
		xlim=(-90,-75),xlocator=-90:2.5:-75,xlabel=L"Longitude / $\degree$",
		ylim=(0,15),ylocator=0:2.5:15,ylabel=L"Latitude / $\degree$",
		suptitle="Colombia Local Time: Hour $(@sprintf("%04.1f",mod(it/2-5,24)))"
	)

	f2.colorbar(c2)
	f2.savefig(plotsdir("01c-prcpanim-$it.png"),transparent=false,dpi=200)
end

# ╔═╡ a24911dc-7b4a-4988-a63b-5797e8b9e283
load(plotsdir("01c-prcpanim-1.png"))
...
begin
	...
	prcpbin_PAC = circshift(prcpbin_PAC,(14,0))
	prcpbin_ATL = circshift(prcpbin_ATL,(14,0))
	prcpbin_PCS = circshift(prcpbin_PCS,(14,0))
	prcpbin_SAN = circshift(prcpbin_SAN,(14,0))
...
begin
	...
	c1 = a1[1].pcolormesh(
		lsmplot,-12:0.5:12,prcpbin_PAC,levels=10. .^(-1:0.1:0.5),
		cmap="blues",extend="both"
	); a1[1].format(ultitle="(a) PAC")
	
	a1[2].pcolormesh(
		lsmplot,-12:0.5:12,prcpbin_ATL,levels=10. .^(-1:0.1:0.5),
		cmap="blues",extend="both"
	); a1[2].format(ultitle="(b) ATL")
	
	a1[3].pcolormesh(
		lsmplot,-12:0.5:12,prcpbin_PCS,levels=10. .^(-1:0.1:0.5),
		cmap="blues",extend="both"
	); a1[3].format(ultitle="(c) PCS")
	
	a1[4].pcolormesh(
		lsmplot,-12:0.5:12,prcpbin_SAN,levels=10. .^(-1:0.1:0.5),
		cmap="blues",extend="both"
	); a1[4].format(ultitle="(d) SAN")

	for ax in a1
		ax.plot([8,8],[-12,12],c="k",linestyle="-")
		ax.text(8.15,9,"Coastline",c="k")
		ax.format(
			ylocator=-12:3:24,ylabel="Hour of Day",
			yticklabels=["12","15","18","21","24","3","6","9","12"],
	...
	f1.colorbar(
		c1,label=L"Rain Rate / mm hr$^{-1}$",ticklabels=["1e-1","1e-0.5","1","1e0.5"],
		locator=10. .^(-1:0.5:1),minorlocator=10. .^(-1:0.1:1),length=0.5
	)
	
	f1.savefig(plotsdir("01c-precipitationbin.png"),transparent=false,dpi=400)
	load(plotsdir("01c-precipitationbin.png"))
end
